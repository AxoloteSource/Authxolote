...
const themeConfigSlice = createSlice({
  ...
  reducers: {
    ...
    setPageTitle(state, { payload }) {
      //TODO: BRANDING obtener el nombre del environment
      document.title = `${payload} | ${i18next.t('brand_name')}`
    },
    syncThemeFromStorage(state, { payload }) {
      // Sync theme from localStorage changes (cross-tab synchronization)
      if (payload) {
        state.theme = payload
        if (payload === 'light') {
          state.isDarkMode = false
